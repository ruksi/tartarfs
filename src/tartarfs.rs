...
impl TartarFS {
    pub fn new(archive_path: String) -> Self {
        ...
        info!("Initialized filesystem with {} items", fs.inode_to_item.len());
        fs
    }

    fn get_file_attributes(&self,  inode: Inode, item: &ArchiveItem) -> FileAttr {
...
impl Filesystem for TartarFS {
    fn lookup(
        ...
        reply.entry(&ttl, &self.get_file_attributes(inode, item), 0);
    }
    ...
    fn getattr(&mut self, _req: &Request, inode_number: u64, _fh: Option<u64>, reply: ReplyAttr) {
        ...
        let ttl = Duration::from_secs(1);
        reply.attr(&ttl, &self.get_file_attributes(Inode(inode_number), item));
    }
